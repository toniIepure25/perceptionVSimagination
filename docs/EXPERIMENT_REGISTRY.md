...
## EXP-2026-04-07-FULL-OVERLAP-SHARED-ONLY-READINESS-AUDIT

- Date: 2026-04-07
- Lane: Animus subsystem engineering
- Benchmark rung / role: explicit readiness-promotion gating for the best current non-smoke canonical neural baseline
- Config: `configs/canonical/full_imagery_overlap_shared_only.yaml`; canonical workflows `./.venv/bin/python -m fmri2img.workflows.eval_decoder --config configs/canonical/full_imagery_overlap_shared_only.yaml --override training.device="cpu" --checkpoint outputs/canonical/train/full_imagery_overlap_shared_only/best_decoder.pt`, `./.venv/bin/python -m fmri2img.workflows.eval_transfer --config configs/canonical/full_imagery_overlap_shared_only.yaml --override training.device="cpu" --checkpoint outputs/canonical/train/full_imagery_overlap_shared_only/best_decoder.pt`, `./.venv/bin/python -m fmri2img.workflows.export_for_animus --config configs/canonical/full_imagery_overlap_shared_only.yaml --override training.device="cpu" --checkpoint outputs/canonical/train/full_imagery_overlap_shared_only/best_decoder.pt`, and `./.venv/bin/python -m fmri2img.workflows.audit_full_imagery_overlap_shared_only_readiness --config configs/canonical/full_imagery_overlap_shared_only.yaml`
- Dataset / prepared artifacts: `outputs/canonical/prepared/full_imagery_overlap/full_imagery_overlap_mixed_with_roi.parquet`, `outputs/targets/full_imagery_overlap_vit_l14_image_768.parquet`, train/eval/transfer/export bundle roots under `outputs/canonical/{train,eval,transfer,export}/full_imagery_overlap_shared_only/`
- Output / artifact path: readiness audit `outputs/canonical/eval/full_imagery_overlap_shared_only/readiness_audit.json`
- Status: done
- Result summary: the live pod readiness report now marks `operational_ready=true`, `downstream_contract_ready=true`, `evidence_ready_candidate=true`, and `training_ready=false` for the checked-in shared-only full-overlap bundle. The export bundle now carries normalized target metadata and normalized condition semantics through `manifest.json` and `decoder_card.json`, and the remaining training blockers are explicit: the train checkpoint still has `max_available_overlap` override provenance and the held-out paired evaluation slice is still only `1/32` paired groups
- Interpretation summary: this is the first real non-smoke, non-fixture promotion lane beyond the smoke ceiling for the practical shared-only path. It strengthens the benchmark-adjacent and Animus-adjacent readiness story without claiming benchmark victory, evidence-grade confirmation, or production deployment readiness
- Promoted to evidence?: no

## EXP-2026-04-07-FULL-OVERLAP-SHARED-ONLY-DEDICATED-RERUN

- Date: 2026-04-07
- Lane: Animus subsystem engineering
- Benchmark rung / role: dedicated checked-in training provenance refresh for the best current non-smoke canonical neural baseline
- Config: `configs/canonical/full_imagery_overlap_shared_only.yaml`; canonical workflows `./.venv/bin/python -m fmri2img.workflows.preflight_data --config configs/canonical/full_imagery_overlap_shared_only.yaml --output outputs/canonical/eval/full_imagery_overlap_shared_only/preflight_data_full_imagery_overlap_shared_only.json`, `./.venv/bin/python -m fmri2img.workflows.train_decoder --config configs/canonical/full_imagery_overlap_shared_only.yaml`, `./.venv/bin/python -m fmri2img.workflows.eval_decoder --config configs/canonical/full_imagery_overlap_shared_only.yaml --checkpoint outputs/canonical/train/full_imagery_overlap_shared_only/best_decoder.pt`, `./.venv/bin/python -m fmri2img.workflows.eval_transfer --config configs/canonical/full_imagery_overlap_shared_only.yaml --checkpoint outputs/canonical/train/full_imagery_overlap_shared_only/best_decoder.pt`, `./.venv/bin/python -m fmri2img.workflows.export_for_animus --config configs/canonical/full_imagery_overlap_shared_only.yaml --checkpoint outputs/canonical/train/full_imagery_overlap_shared_only/best_decoder.pt`, and `./.venv/bin/python -m fmri2img.workflows.audit_full_imagery_overlap_shared_only_readiness --config configs/canonical/full_imagery_overlap_shared_only.yaml`
- Dataset / prepared artifacts: `outputs/canonical/prepared/full_imagery_overlap/full_imagery_overlap_mixed_with_roi.parquet`, `outputs/targets/full_imagery_overlap_vit_l14_image_768.parquet`
- Output / artifact path: refreshed train/eval/transfer/export roots under `outputs/canonical/{train,eval,transfer,export}/full_imagery_overlap_shared_only/`; refreshed readiness audit at `outputs/canonical/eval/full_imagery_overlap_shared_only/readiness_audit.json`
- Status: done
- Result summary: the live pod now has a dedicated checked-in train provenance for `full_imagery_overlap_shared_only`. The refreshed `config_snapshot.json` records `experiment.name="full_imagery_overlap_shared_only"` and the readiness audit now removes the old provenance blocker entirely. The refreshed report still marks `operational_ready=true`, `downstream_contract_ready=true`, `evidence_ready_candidate=true`, and `training_ready=false`, with the only remaining blocker being `held-out paired evaluation support remains too small for training-ready promotion (1/32 paired groups)`
- Interpretation summary: this pass truthfully advances the best current non-smoke shared-only lane beyond the previous smoke-only and override-provenance ceiling, but it does not change the evidence freeze, benchmark ranking, or production readiness. It only removes blocker `#1`
- Promoted to evidence?: no

## EXP-2026-04-07-FULL-OVERLAP-SHARED-ONLY-HELDOUT-CEILING

- Date: 2026-04-07
- Lane: Animus subsystem engineering
- Benchmark rung / role: held-out paired-support ceiling audit for the best current non-smoke canonical neural baseline
- Config: `configs/canonical/full_imagery_overlap_shared_only.yaml`; canonical workflow `./.venv/bin/python -m fmri2img.workflows.audit_full_imagery_overlap_shared_only_readiness --config configs/canonical/full_imagery_overlap_shared_only.yaml --output outputs/canonical/eval/full_imagery_overlap_shared_only/readiness_audit.json`; temporary exact-config rebuild proof via `./.venv/bin/python -m fmri2img.workflows.prepare_overlap_bootstrap --config configs/canonical/full_imagery_overlap_shared_only.yaml --overwrite-existing --override preparation.overlap.perception_index_template='"/home/jovyan/work/FMRI2images/data/indices/nsd_index/subject={subject}/index.parquet"' --override preparation.overlap.mask_root_template='"outputs/canonical/prepared/roi_masks/{subject}"' --override dataset.mixed_index='"outputs/canonical/prepared/full_imagery_overlap_phase3_audit/full_imagery_overlap_mixed_with_roi.parquet"' --override preparation.overlap.output_root='"outputs/canonical/prepared/full_imagery_overlap_phase3_audit"' --override preparation.overlap.report_path='"outputs/canonical/prepared/full_imagery_overlap_phase3_audit/report.json"' --override preparation.overlap.overlap_ids_path='"outputs/canonical/prepared/full_imagery_overlap_phase3_audit/overlap_nsd_ids.json"'`
- Dataset / prepared artifacts: canonical prepared overlap bundle at `outputs/canonical/prepared/full_imagery_overlap/full_imagery_overlap_mixed_with_roi.parquet`; temporary exact-config rebuild proof at `outputs/canonical/prepared/full_imagery_overlap_phase3_audit/`
- Output / artifact path: refreshed readiness audit `outputs/canonical/eval/full_imagery_overlap_shared_only/readiness_audit.json`; temporary rebuild proof `outputs/canonical/prepared/full_imagery_overlap_phase3_audit/{report.json,overlap_nsd_ids.json,full_imagery_overlap_mixed_with_roi.parquet}`
- Status: done
