...
\paragraph{Lack of statistical breadth.}
The frozen results come from a single run per model configuration without
repeated trials or error bars.  This limits our ability to assess variability
and means the benchmark ordering should be interpreted as a frozen snapshot
rather than a statistically confirmed ranking.
...
with \texttt{seed=0}.  The supplementary material includes exact instructions
for reproducing the benchmark with alternative seeds.  The ordering gap between
shared-only (cosine 0.136) and the best shared-private variant (0.108) is large
relative to typical seed variation on datasets of this scale, but we do not
claim statistical confirmation of the ordering.  Seed-stability verification
remains an open item that reviewers can execute from the provided configs and
commands.
